  ...
  def collisions(self):
    ...
    for cart in self.carts:
      if cart.track.position in cart_positions:
        collisions.append(cart.track.position)
      else:
        cart_positions.add(cart.track.position)
    return collisions
  
  def remove_carts(self, positions):
    for position in positions:
      self.carts = list(filter(lambda c : c.track.position != position, self.carts))
